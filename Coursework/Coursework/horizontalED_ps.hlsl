...
float4 main(InputType input) : SV_TARGET
{
    ...
    colour += shaderTexture.Sample(SampleType, input.tex + float2(texelSize * 0.0f, texelSize * 1.0f)) * 2.0; // bottom middle
    colour += shaderTexture.Sample(SampleType, input.tex + float2(texelSize * 1.0f, texelSize * 1.0f)) * 1.0; // bottom right
    colour += shaderTexture.Sample(SampleType, input.tex + float2(texelSize * -1.0f, texelSize * 0.0f)) * 0.0; // left
    colour += shaderTexture.Sample(SampleType, input.tex) * 0.0; // middle
    colour += shaderTexture.Sample(SampleType, input.tex + float2(texelSize * 1.0f, texelSize * 0.0f)) * 0.0; // right
    colour += shaderTexture.Sample(SampleType, input.tex + float2(texelSize * -1.0f, texelSize * -1.0f)) * -1.0; // top left
    colour += shaderTexture.Sample(SampleType, input.tex + float2(texelSize * 0.0f, texelSize * -1.0f)) * -2.0; // top middle
