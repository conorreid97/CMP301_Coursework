...
MountainShader::MountainShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"mountain_vs.cso", L"mountain_ps.cso");
}
...
void MountainShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
...
void MountainShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, 
										const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture0, ID3D11ShaderResourceView* texture1, 
										ID3D11ShaderResourceView* texture2, ID3D11ShaderResourceView* textureH, Light* light[3]/*, Light* light1*/, float timer, XMFLOAT3 cameraPos)
{
	HRESULT result;
	...
	/*for (int i = 0; i < 3; i++) {
		deviceContext->PSSetShaderResources(0, 3, &texArr[i]);
	}*/

	/*deviceContext->PSSetShaderResources(0, 3, &texture0);
...
void MountainShader::setAmplitude(float amp_) {
	myAmplitude_ = amp_;
	//return timePtr->amplitude;
}
float MountainShader::getAmplitude() {
	return myAmplitude_;
}

void MountainShader::setSpeed(float speed_) {
	mySpeed_ = speed_;
}

float MountainShader::getSpeed() {
	return mySpeed_;
}

void MountainShader::setFrequency(float freq_) {
	myFrequency_ = freq_;
}

float MountainShader::getFrequency() {
	return myFrequency_;
}

void MountainShader::setNormalToggle(float tog_) {
	normalToggle_ = tog_;
}

float MountainShader::getNormalToggle() {
	return myFrequency_;
}